...
import { Home, Trophy, Users, User as UserIcon, Settings, Zap, Activity, Gift, RefreshCcw, Terminal, ChevronLeft, ChevronRight, Menu, Bell, Sun, Moon } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const [theme, setTheme] = useState<'dark' | 'light'>(() => {
    const savedTheme = localStorage.getItem('funpace_theme');
    return (savedTheme === 'light' || savedTheme === 'dark') ? savedTheme : 'dark';
  });
  const [isRailExpanded, setIsRailExpanded] = useState(false);

  useEffect(() => {
    localStorage.setItem('funpace_theme', theme);
    if (theme === 'dark') {
  ...
  return (
    <div className="min-h-screen flex overflow-hidden transition-colors duration-300 bg-slate-50 text-slate-900 dark:bg-[#020617] dark:text-slate-100">
      ...
      <aside 
        ...
        className={`hidden md:flex flex-col z-[100] transition-all duration-500 ease-in-out border-r border-slate-200 dark:border-white/5 bg-white/80 dark:bg-slate-950/80 backdrop-blur-2xl ${isRailExpanded ? 'w-64' : 'w-24'}`}
      >
        <button 
          onClick={handleLogout}
          className="p-6 flex items-center gap-4 w-full hover:bg-slate-100 dark:hover:bg-white/5 transition-colors text-left"
        >
           ...
           {navItems.map(item => (
             <button
               ...
               className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${
                 activeTab === item.id 
                 ? 'bg-slate-100 dark:bg-white/5 text-slate-900 dark:text-white ring-1 ring-slate-200 dark:ring-white/10 shadow-xl' 
        ...
        <div className="p-4 space-y-4">
           <button 
             ...
             className="w-full flex items-center gap-4 p-4 rounded-2xl transition-all text-slate-500 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5"
           >
              {theme === 'dark' ? <Sun className="w-6 h-6 flex-shrink-0" /> : <Moon className="w-6 h-6 flex-shrink-0" />}
              {isRailExpanded && <span className="text-[10px] font-black uppercase tracking-widest">{theme === 'dark' ? 'Light Mode' : 'Dark Mode'}</span>}
           </button>

           <button 
             onClick={() => setActiveTab('sync')}
             className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${activeTab === 'sync' ? 'bg-orange-600/20 text-orange-600 dark:text-orange-400' : 'text-slate-500 hover:text-orange-500'}`}
           >
           ...
           <button 
             ...
             className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${activeTab === 'system' ? 'bg-cyan-600/20 text-cyan-600 dark:text-cyan-400' : 'text-slate-500 hover:text-cyan-400'}`}
           >
      ...
      <main className="flex-1 flex flex-col relative z-10 overflow-hidden">
        ...
        <header className="px-8 py-6 flex justify-between items-center">
           <div className="flex items-center gap-4">
              <button className="md:hidden p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white">
           ...
           <div className="flex items-center gap-6">
              <div className="flex items-center gap-3 bg-white/50 dark:bg-slate-900/50 px-4 py-2 rounded-2xl border border-slate-200 dark:border-white/5">
                 <div className="w-2 h-2 bg-lime-500 rounded-full animate-pulse" />
                 <span className="text-[9px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400">Mainframe_Linked</span>
              </div>
              <button className="relative p-2 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">
                 <Bell className="w-5 h-5" />
                 <div className="absolute top-1 right-1 w-2 h-2 bg-pink-500 rounded-full border-2 border-white dark:border-slate-950" />
              </button>
              <div className="w-10 h-10 rounded-xl overflow-hidden border border-slate-200 dark:border-white/10 ring-2 ring-cyan-500/20">
        ...
        <nav className="md:hidden fixed bottom-6 left-6 right-6 z-[100] bg-white/90 dark:bg-slate-900/90 backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-[2rem] p-3 flex justify-around shadow-2xl">
           {navItems.slice(0, 5).map(item => (
             <button 
               key={item.id} 
               onClick={() => setActiveTab(item.id as any)}
               className={`p-3 rounded-2xl ${activeTab === item.id ? 'bg-slate-100 dark:bg-white/10 ' + item.color : 'text-slate-500'}`}
             >
