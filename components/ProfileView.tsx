      ...
      <div className={`relative overflow-hidden rounded-[3.5rem] p-10 border-2 ${isDark ? 'bg-slate-900/80 border-slate-800' : 'bg-white border-slate-200 shadow-2xl'}`}>
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
          <div className="relative group">
            ...
            <div className={`absolute -bottom-2 -right-2 bg-blue-600 p-3 rounded-2xl border-4 ${isDark ? 'border-slate-900' : 'border-white'} shadow-2xl`}>
          ...
          <div className="flex gap-3">
             <button onClick={onToggleTheme} className={`p-4 rounded-2xl border transition-colors ${isDark ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-slate-100 border-slate-200 hover:bg-slate-200'}`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all whitespace-nowrap border-2 ${
              activeTab === tab.id 
                ...
                : isDark ? 'bg-slate-900 border-slate-800 text-slate-500 hover:border-slate-700' : 'bg-slate-100 border-slate-200 text-slate-500 hover:border-slate-300'
            }`}
          >
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="space-y-4">
              {activityHistory.map(activity => (
                <div key={activity.id} className={`flex items-center justify-between p-5 border rounded-3xl group hover:border-blue-500/50 transition-all ${isDark ? 'bg-slate-950/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
              ...
              {MOCK_BADGES.map(badge => (
                <div 
                  ...
                  className={`relative p-6 rounded-3xl border-2 flex flex-col items-center text-center gap-3 transition-all ${
                    badge.unlockedAt 
                      ? isDark ? 'bg-slate-900/60 border-slate-800 hover:border-blue-500 hover:-translate-y-1' : 'bg-slate-50 border-slate-200 hover:border-blue-500 hover:-translate-y-1'
