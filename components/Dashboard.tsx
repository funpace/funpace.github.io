    ...
    <div className="bento-grid animate-in fade-in duration-1000">
      ...
      <div className="lg:col-span-8 md:col-span-12 glass-panel p-10 flex flex-col md:flex-row items-center gap-10 overflow-hidden relative group">
         ...
         <div className="relative">
            <div className="w-40 h-40 rounded-[3rem] p-1 bg-gradient-to-tr from-cyan-400 to-pink-500 shadow-[0_0_40px_rgba(6,182,212,0.2)]">
               <div className="w-full h-full rounded-[2.8rem] overflow-hidden bg-white dark:bg-slate-950 border-4 border-white dark:border-slate-950">
            ...
            <div className="absolute -bottom-2 -right-2 bg-pink-600 text-white font-black text-xs px-4 py-2 rounded-2xl border-4 border-white dark:border-slate-950">
               N0{stats.phase}
            </div>
         </div>
         ...
         <div className="flex-1 text-center md:text-left space-y-4">
            <div>
               ...
               <h2 className="text-5xl font-black italic uppercase tracking-tighter text-slate-900 dark:text-white">Alex <span className="text-pink-500">Core</span></h2>
               <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">{stats.title}</p>
            </div>
            <div className="flex flex-wrap justify-center md:justify-start gap-3">
               <span className="bg-white/80 dark:bg-slate-900/80 px-4 py-2 rounded-xl text-[10px] font-black uppercase text-cyan-600 dark:text-cyan-400 border border-cyan-500/20">STREAK: {stats.streak}D</span>
               <span className="bg-white/80 dark:bg-slate-900/80 px-4 py-2 rounded-xl text-[10px] font-black uppercase text-pink-600 dark:text-pink-400 border border-pink-500/20">DIST: {stats.totalDistance}KM</span>
            </div>
         </div>
         ...
         <div className="hidden lg:flex flex-col items-end gap-2 text-right">
            ...
            <div className="flex gap-1">
               {[1, 2, 3, 4, 5].map(i => (
                 <div key={i} className={`w-1.5 h-6 rounded-full ${i <= 4 ? 'bg-cyan-500 shadow-[0_0_10px_#06b6d4]' : 'bg-slate-200 dark:bg-slate-800'}`} />
               ))}
            </div>
            <p className="text-xs font-black italic text-cyan-600 dark:text-cyan-400">SYNCHRONIZED</p>
         </div>
      </div>
      ...
      <div className="lg:col-span-7 md:col-span-12 glass-panel p-8 group relative bg-lime-500/[0.03] border-l-4 border-lime-500">
         <div className="flex items-center gap-8">
            ...
            <div className="space-y-2">
               ...
               {isNarratorLoading ? (
                  <div className="h-4 w-64 bg-slate-200 dark:bg-slate-800 animate-pulse rounded" />
               ) : (
                  <p className="text-xl font-bold italic text-slate-700 dark:text-slate-200 group-hover:text-slate-900 dark:group-hover:text-white transition-colors leading-relaxed">"{narratorText}"</p>
               )}
            </div>
         </div>
      </div>
      ...
      <div className="lg:col-span-5 md:col-span-6 glass-panel p-10 flex flex-col justify-between border-r-4 border-cyan-500 glow-cyan">
         ...
         <div className="relative h-24 flex items-end">
            ...
            <div className="w-full space-y-4">
               <div className="flex justify-between text-[10px] font-black uppercase text-cyan-600 dark:text-cyan-400">
                  <span>Sync Fidelity</span>
                  <span>{stats.neuralSync} NS</span>
               </div>
               <div className="h-4 bg-slate-100 dark:bg-slate-950 rounded-full overflow-hidden p-1 border border-slate-200 dark:border-white/5">
      ...
      <div className="col-span-12 glass-panel p-10 space-y-8">
         <div className="flex justify-between items-center">
            <h3 className="text-2xl font-black italic uppercase flex items-center gap-3 text-slate-900 dark:text-white">
               <Signal className="w-6 h-6 text-cyan-400" /> Upcoming Operations
            </h3>
            <button 
              onClick={() => onNavigateToTab('workouts')}
              className="text-[10px] font-black uppercase text-cyan-500 tracking-widest flex items-center gap-2 hover:text-slate-900 dark:hover:text-white transition-colors"
            >
         ...
         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {MOCK_TRAININGS.map(t => (
               <div key={t.id} className="p-6 bg-white/60 dark:bg-slate-900/60 rounded-3xl border border-slate-200 dark:border-white/5 group hover:border-cyan-500/30 transition-all cursor-pointer">
                  ...
                  <h4 className="text-xl font-black italic uppercase text-slate-900 dark:text-white group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">{t.title}</h4>
                  ...
                  <div className="flex justify-between items-center">
                     <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-black text-slate-900 dark:text-white">+{t.funitsReward}F</span>
                     </div>
                     <ChevronRight className="w-5 h-5 text-slate-400 dark:text-slate-700 group-hover:text-slate-900 dark:group-hover:text-white group-hover:translate-x-1 transition-all" />
                  </div>
               </div>
            ))}
         </div>
