      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={item.name} className={`flex items-start gap-4 p-6 rounded-2xl border transition-all group hover:border-blue-500/50 ${isDark ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-700">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-slate-800 pb-8">
        ...
        <div className={`flex items-center gap-4 px-6 py-2 rounded-full border ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-slate-100 border-slate-200'}`}>
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-6 py-4 rounded-2xl border font-black text-[10px] uppercase tracking-widest transition-all flex-shrink-0 ${
              activeTab === tab.id 
                ...
                : isDark ? 'border-slate-800 bg-slate-900/40 text-slate-500 hover:border-slate-600' : 'border-slate-200 bg-slate-100 text-slate-500 hover:border-slate-300'
            }`}
          >
